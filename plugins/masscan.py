...
# Required
REQUIRED_TOOL = "masscan"     # e.g., "nmap", "gobuster", "rustscan", "nuclei"
INSTALL_HINT  = "apt"         # one: "apt" | "brew" | "choco" | "pip" | "go" | "git" | "manual" | "docker"

# Optional
INSTALL_URL   = "https://github.com/robertdavidgraham/masscan"  # Docs/repo page (shown to user for manual/git)
...
DEFAULT_ARGS = {
    # Fast full-range scan; tune --rate per environment. --wait 0 reduces tail wait.
    "Aggressive": "-p1-65535 --rate 50000 --wait 0 {{target}}",
    # Quicker sweep of common ports (uses masscan's --top-ports)
    "Normal":     "--top-ports 1000 --rate 10000 --wait 0 {{target}}",
    # Masscan is inherently active; keep Passive disabled by default
