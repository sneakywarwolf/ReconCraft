...
# Required
REQUIRED_TOOL = "subfinder"   # e.g., "nmap", "gobuster", "rustscan", "nuclei"
INSTALL_HINT  = "go"          # one: "apt" | "brew" | "choco" | "pip" | "go" | "git" | "manual" | "docker"

# Optional
INSTALL_URL   = "https://github.com/projectdiscovery/subfinder"  # Docs/repo page (shown to user for manual/git)
...
DEFAULT_ARGS = {
    # Broad/recursive with active resolution and higher concurrency
    "Aggressive": "-d {{target}} -all -recursive -active -t 50 -timeout 10 -silent",
    # Standard passive+resolution with moderate threads
    "Normal":     "-d {{target}} -all -t 25 -silent",
    # Strictly passive sources only (no active resolution)
    "Passive":    "-d {{target}} -passive -silent",
