...
# Required
REQUIRED_TOOL = 'rustscan'    # e.g., "nmap", "gobuster", "rustscan", "nuclei"
INSTALL_HINT  = 'docker'         # one: "apt" | "brew" | "choco" | "pip" | "go" | "git" | "manual" | "docker"

# Optional
INSTALL_URL   = ''            # Docs/repo page (shown to user for manual/git)
EXECUTABLE    = ''            # How ReconCraft should call it; default = REQUIRED_TOOL
TOOL_ALIAS    = 'rustscan'            # Name of shim/alias to create (used esp. for Docker); default = EXECUTABLE or REQUIRED_TOOL

# Optional (ONLY if INSTALL_HINT == "docker")

# Example (RustScan): "docker run -it --rm --name rustscan rustscan/rustscan:alpine"
DOCKER_RUN    = 'docker run --rm -i --name rustscan rustscan/rustscan:alpine'            # Leave empty unless you want Docker-backed install
